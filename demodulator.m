...
function [a_new] = demodulator(r,Modulation)
...
if (Modulation == 2) % QPSK
    inPhase = limits( real( sqrt(2)*r ) ); 
    quadrature = limits( imag( sqrt(2)*r ) ); 
...
if (Modulation == 6) % 64_QAM
    ...
    a_new = [ limits( inPhaseBit1' ); limits( inPhaseBit2' ); limits( inPhaseBit3' ); ...
        limits( quadratureBit1' ); limits( quadratureBit2' ); limits( quadratureBit3' ) ];
    a_new = reshape( a_new, 1, [] );
